...
class local_planar_guidance(nn.Module):
    ...
    def forward(self, plane_eq, focal):
        ...
        plane = plane_eq_expanded[:, 0:3, :, :]

        return n4 / (n1 * u + n2 * v + n3), plane
...
class bts(nn.Module):
    def __init__(self, params, feat_out_channels, num_features=512):
        ...
        self.refine_plane = depth_to_normal()

    def forward(self, features, seg_map, k_inv_dot_xy1, focal):
        ...
        depth_8x8, plane_8x8 = self.lpg8x8(plane_eq_8x8, focal)
        ...
        depth_4x4, plane_4x4 = self.lpg4x4(plane_eq_4x4, focal)
        ...
        depth_2x2, plane_2x2 = self.lpg2x2(plane_eq_2x2, focal)
        ...
        plane1x1 = self.plane1x1(upconv1)
        plane_normal_1x1 = plane1x1[:, :3, :, :]
        ...
        plane_normal = self.get_planes(last_feat)

        final_plane_normal = self.refine_plane(k_inv_dot_xy1, plane_normal, final_depth, seg_map)
...
class depth_to_normal(nn.Module):
    def __init__(self):
        super(depth_to_normal, self).__init__()
        ...
        self.refine_module = torch.nn.Sequential(
    ...
    def refine(self, k_inv_dot_xy1, seg_map, depth_map):
    ...
    def forward(self, k_inv_dot_xy1, init_normal, depth_map, seg_map):
        n, c, h, w = depth_map.shape
        rough_normals = []
        for i in range(n):
            normals = self.refine(k_inv_dot_xy1, seg_map[i], depth_map[i].squeeze())
            rough_normals.append(normals)
        rough_normals = torch.stack(rough_normals, dim=0)

        normals_clean = self.filter_module(rough_normals)
        normals_add = torch.add(normals_clean, rough_normals)
        normals_add = torch_nn_func.normalize(normals_add, 2, 1)

        normals_cat = torch.cat([init_normal, normals_add], dim=1)
        normals_refine = self.refine_module(normals_cat)
        return normals_refine
...
class BtsModel(nn.Module):
    ...
    def forward(self, x, seg_map, k_inv_dot_xy1, focal):
        skip_feat = self.encoder(x)
        return self.decoder(skip_feat, seg_map, k_inv_dot_xy1, focal)
